...
int Solution::MatrixMul(const vector<long long> &p, vector<vector<long long>> &M, vector<vector<int>> &K)
{
    // 确定联乘矩阵个数
    int n = p.size() - 1;
...
// 输出A[s]~A[t]的联乘式
int Solution::Output(const vector<vector<int>> &K, int s, int t)
{
    if (s == t) {
        cout << 'A' << s;
    } else {
